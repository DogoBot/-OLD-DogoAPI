...
class ResourceController {
    ...
    @PostMapping("/token/")
    fun tokenPost(@RequestBody newtoken : String, @RequestParam("token", required = false) token : String?) : ResponseEntity<*>{
        val response = DogoAPI.connection?.request(JSONObject().put("token", token).put("iwant", "addtoken").put("newtoken", newtoken))
        return ResponseEntity(response.toString(), HttpStatus.OK)
    }
